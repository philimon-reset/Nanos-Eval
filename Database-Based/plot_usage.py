...
def plot_database_metric(nanos_df, docker_df, field_pair):
    def prepare_data(df):
        ...
        for connection in [10, 50, 100]:
            get_data = df[(df['connection'] == connection) & (df['test'] == 'GET')]
            set_data = df[(df['connection'] == connection) & (df['test'] == 'SET')]
            result[str(connection)]['min'] = {'GET': get_data[field_pair[0]].min(), 'SET': set_data[field_pair[0]].min()}
            result[str(connection)]['max'] = {'GET': get_data[field_pair[0]].max(), 'SET': set_data[field_pair[0]].max()}
            result[str(connection)]['mean'] = {'GET': get_data[field_pair[0]].mean(), 'SET': set_data[field_pair[0]].mean()}
        return result
...
def plot_box_plot(data, field_pair, path):
    ...
    connection_levels = [('GET',10), ('SET',10), ('GET',50), ('SET',50), ('GET',100), ('SET',100)]
    data = [data[(data["connection"] == level[1]) & (data['test'] == level[0])][field_pair[0]].dropna().tolist() for level in connection_levels]
    operation_colors = {
        'GET': 'orange',
        'SET': 'tomato'
    }
    ...
    ax.set_title(f"Distribution of {field_pair[1]} by Connection Count and Operation.")

    legend_patches = [mpatches.Patch(color=color, label=label) for label, color in operation_colors.items()]
...
def plot_bar_with_error(nanos_data, docker_data, field_pair, path):
    ...
    nanos_means = [nanos_data[str(conn)]['mean'][op] for op, conn in connection_levels]
    docker_means = [docker_data[str(conn)]['mean'][op] for op, conn in connection_levels]
    ...
    rects1 = ax.bar(x - width/2 - 0.02, nanos_means, width, label='Nanos', color='orange')
    rects2 = ax.bar(x + width/2 + 0.02, docker_means, width, label='Docker', color='blue')
...
def compare_database_metrics_plot(nanos_process, docker_process):
    ...
    nanos_df.rename(columns={'rps': 'requests_per_sec', 'avg_latency_ms': 'latency_avg'}, inplace=True)
    docker_df.rename(columns={'rps': 'requests_per_sec', 'avg_latency_ms': 'latency_avg'}, inplace=True)
